...
public final class ActivityThread {
    ...
    static final class ActivityClientRecord {
        ...
        // Added by Franzi
        IBinder containerToken;
        IBinder containerWindowToken;
        Bundle embeddedLayoutParams;
        int embeddedActivityId;
        int lastKnownHeight;
        int lastKnownWidth;
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        // Franzi
        public final void scheduleLaunchEmbeddedActivity(Intent intent, IBinder token, int ident,
                ActivityInfo info, CompatibilityInfo compatInfo, Bundle state, IBinder containerToken,
                IBinder containerWindowToken, int embeddedActivityId, Bundle layoutParams) {
            ActivityClientRecord r = new ActivityClientRecord();
            ...
            r.compatInfo = compatInfo;
            r.state = state;
            r.containerToken = containerToken;
            r.containerWindowToken = containerWindowToken;
            r.embeddedActivityId = embeddedActivityId;
            r.embeddedLayoutParams = layoutParams;
            r.lastKnownWidth = 0;
            r.lastKnownHeight = 0;

            queueOrSendMessage(H.LAUNCH_EMBEDDED_ACTIVITY, r);
        }
        
        // Franzi 
        public final void scheduleRefreshEmbeddedActivity(IBinder token, Bundle layoutParams) {
            ActivityClientRecord r = mActivities.get(token);
            r.embeddedLayoutParams = layoutParams; // Layout params may have changed
            queueOrSendMessage(H.REFRESH_EMBEDDED_ACTIVITY, r);
        }
        
        // Franzi 
        public final void scheduleRemoveEmbeddedActivity(IBinder token) {
            ActivityClientRecord r = mActivities.get(token);
            queueOrSendMessage(H.REMOVE_EMBEDDED_ACTIVITY, r);
        }
    ...
    private class H extends Handler {
        ...
        public static final int LAUNCH_EMBEDDED_ACTIVITY = 143;
        public static final int REFRESH_EMBEDDED_ACTIVITY = 144;
        public static final int REMOVE_EMBEDDED_ACTIVITY = 145;
        String codeToString(int code) {
            if (DEBUG_MESSAGES) {
                switch (code) {
                    ...
                    case LAUNCH_EMBEDDED_ACTIVITY: return "LAUNCH_EMBEDDED_ACTIVITY";
                    case REFRESH_EMBEDDED_ACTIVITY: return "REFRESH_EMBEDDED_ACTIVITY";
                    case REMOVE_EMBEDDED_ACTIVITY: return "REMOVE_EMBEDDED_ACTIVITY";
                }
            }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
	            case REFRESH_EMBEDDED_ACTIVITY: {
	                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activityRefreshEmbedded");
	                ActivityClientRecord r = (ActivityClientRecord)msg.obj;
	                handleRefreshEmbeddedActivity(r);
	                Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
	            } break;
	            case REMOVE_EMBEDDED_ACTIVITY: {
	                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activityRemoveEmbedded");
	                ActivityClientRecord r = (ActivityClientRecord)msg.obj;
	                handleRemoveEmbeddedActivity(r);
	                Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
	            } break;
	            case LAUNCH_EMBEDDED_ACTIVITY: {
	                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "activityStartEmbedded");
	                ActivityClientRecord r = (ActivityClientRecord)msg.obj;
	                
	                r.packageInfo = getPackageInfoNoCheck(
	                        r.activityInfo.applicationInfo, r.compatInfo);
	                handleLaunchEmbeddedActivity(r, null);
	                Trace.traceEnd(Trace.TRACE_TAG_ACTIVITY_MANAGER);
	            } break;
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
    	...
    	Log.i(TAG, "performLaunchActivity " + r.activityInfo.name + " with containerToken: " + r.containerToken);
    ...
    // Added by Franzi
    private void handleRefreshEmbeddedActivity(ActivityClientRecord r) {
    	Activity a = r.activity;
        View decor = r.window.getDecorView();
        ViewManager wm = a.getWindowManager();
        WindowManager.LayoutParams l = r.window.getAttributes();
        l.type = WindowManager.LayoutParams.TYPE_EMBEDDED_ACTIVITY;
        l.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL;
        l.token = r.containerWindowToken;
        l.selfToken = r.token;
        l.realParentToken = r.containerToken;
        l.x = r.embeddedLayoutParams.getInt("x");
        l.y = r.embeddedLayoutParams.getInt("y");
        l.width = r.embeddedLayoutParams.getInt("width");
        l.height = r.embeddedLayoutParams.getInt("height");
        wm.updateViewLayout(decor, l);
        
        // If this Activity didn't get it's request min width/height, notify user.
        int width = r.embeddedLayoutParams.getInt("width");
        int height = r.embeddedLayoutParams.getInt("height");
        a.setExpectedWindowDimensions(height, width); // for clickjacking (fully visible detection)
        int reqWidth = r.activityInfo.minEmbedWidth;
        int reqHeight = r.activityInfo.minEmbedHeight;
        if (width < reqWidth || height < reqHeight) {
        	Log.e(TAG, "Bad embedded size (h,w): (" + height + "," + width + 
        			"), requested (" + reqHeight + "," + reqWidth + ")");
        	notifyUserOfSizeConflict(r, width, height);
        	a.setSizeConflict(true);
        } else {
        	a.setSizeConflict(false);
        }
    }
    
    // Added by Franzi
    private void notifyUserOfSizeConflict(ActivityClientRecord r, int width, int height) {
    	// Don't issue excessive notifications
    	if (width == r.lastKnownWidth && height == r.lastKnownHeight) return;
    	
    	r.lastKnownHeight = height;
    	r.lastKnownWidth = width;
    	
    	Context context = r.activity;
    	int icon = r.activityInfo.icon;
    	if (icon == 0) {
    		icon = r.activityInfo.applicationInfo.icon;
    	}
    	// Issue system notification
    	Notification.Builder mBuilder =
    	        new Notification.Builder(context)
    			.setSmallIcon(icon)
    	        .setContentTitle("Embedded Size Conflict")
    	        //.setContentText("The Activity " + r.activityInfo.name 
    	        //		+ " didn't get its minimum requested size.")
    	        .setContentText(r.activityInfo.loadLabel(r.activity.getPackageManager()))
    	        .setAutoCancel(true);
    	// When the user clicks on this notification, launch the Activity fullscreen.
    	...
    	NotificationManager mNotificationManager =
    		...
    		mNotificationManager.notify(r.embeddedActivityId /* unique to this activity */, mBuilder.build());
    }
    
    // Added by Franzi
    private void handleRemoveEmbeddedActivity(ActivityClientRecord r) {
    	// This is just the same as destroying an Activity.
    	// If r is null, might have already destroyed for some other reason
    	if (r != null) {
    		handleDestroyActivity(r.token, true, 0, false); // TODO: not sure all args are appropriate
    	}
    }
    
    // Added by Franzi
    private void handleLaunchEmbeddedActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        // If this Activity didn't get its requested min width/height, notify user.
        int width = r.embeddedLayoutParams.getInt("width");
        int height = r.embeddedLayoutParams.getInt("height");
        a.setExpectedWindowDimensions(height, width); // for clickjacking (fully visible detection)
        int reqWidth = r.activityInfo.minEmbedWidth;
    ...
    final void handleResumeActivity(IBinder token, boolean clearHide, boolean isForward,
        ...
        if (r != null) {
            ...
            if (r.containerToken == null) { // only for non-embedded activities; AM doesn't known about embeddeds 
	            if (reallyResume) {
	                try {
            ...
            try {
                ...
                // Franzi's note: this will get called even if it's an embedded
                // activity, but ActivityManager will fail gracefully when it can't find
                // that Activity in its history stack.
            } catch (RemoteException ex) {
            }
        }
    }
        ...
        if (finishing) {
            try {
            	if (r.containerToken == null) { // only for non-embedded activities; AM doesn't known about embeddeds
            		...
            		ActivityManagerNative.getDefault().childActivityDestroyed(r.containerToken, token);
            	}
            } catch (RemoteException ex) {
