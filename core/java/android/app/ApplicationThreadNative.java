...
public abstract class ApplicationThreadNative extends Binder
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags)
        ...
        case SCHEDULE_REFRESH_EMBEDDED_ACTIVITY_TRANSACTION:
        {
            data.enforceInterface(IApplicationThread.descriptor);
            IBinder b = data.readStrongBinder();
            Bundle layoutParams = data.readBundle();
            scheduleRefreshEmbeddedActivity(b, layoutParams);
            return true;
        }
        
        case SCHEDULE_REMOVE_EMBEDDED_ACTIVITY_TRANSACTION:
        {
            data.enforceInterface(IApplicationThread.descriptor);
            IBinder b = data.readStrongBinder();
            scheduleRemoveEmbeddedActivity(b);
            return true;
        }
        
        case SCHEDULE_LAUNCH_EMBEDDED_ACTIVITY_TRANSACTION:
        {
            data.enforceInterface(IApplicationThread.descriptor);
            Intent intent = Intent.CREATOR.createFromParcel(data);
            IBinder b = data.readStrongBinder();
            ...
            CompatibilityInfo compatInfo = CompatibilityInfo.CREATOR.createFromParcel(data);
            Bundle state = data.readBundle();
            IBinder container = data.readStrongBinder();
            IBinder containerWindow = data.readStrongBinder();
            int embeddedActivityId = data.readInt();
            Bundle layoutParams = data.readBundle();
            scheduleLaunchEmbeddedActivity(intent, b, ident, info, compatInfo, state, container, 
            		containerWindow, embeddedActivityId, layoutParams);
            return true;
        }
...
class ApplicationThreadProxy implements IApplicationThread {
    ...
    // Added by Franzi
 	public void scheduleRemoveEmbeddedActivity(IBinder token) throws RemoteException {
 		Parcel data = Parcel.obtain();
        data.writeInterfaceToken(IApplicationThread.descriptor);
        data.writeStrongBinder(token);
        mRemote.transact(SCHEDULE_REMOVE_EMBEDDED_ACTIVITY_TRANSACTION, data, null,
                IBinder.FLAG_ONEWAY);
        data.recycle();
 	}

 	// Added by Franzi
 	public void scheduleRefreshEmbeddedActivity(IBinder token, Bundle layoutParams)
 			throws RemoteException {
 		Parcel data = Parcel.obtain();
        data.writeInterfaceToken(IApplicationThread.descriptor);
        data.writeStrongBinder(token);
        data.writeBundle(layoutParams);
        mRemote.transact(SCHEDULE_REFRESH_EMBEDDED_ACTIVITY_TRANSACTION, data, null,
                IBinder.FLAG_ONEWAY);
        data.recycle(); 		
 	}
    ...
    public final void scheduleLaunchEmbeddedActivity(Intent intent, IBinder token, int ident,
            ...
            IBinder containerWindowToken, int embeddedId, Bundle layoutParams) 
    		throws RemoteException {
    	Parcel data = Parcel.obtain();
        data.writeInterfaceToken(IApplicationThread.descriptor);
        ...
        data.writeStrongBinder(containerWindowToken);
        data.writeInt(embeddedId);
        data.writeBundle(layoutParams);
        mRemote.transact(SCHEDULE_LAUNCH_EMBEDDED_ACTIVITY_TRANSACTION, data, null,
                IBinder.FLAG_ONEWAY);
        data.recycle();
    }
