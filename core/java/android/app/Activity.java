...
public class Activity extends ContextThemeWrapper
    ...
    private static final boolean BENCHMARK_EMBEDDED = false;
    ...
    // Added by Franzi
    private boolean mHasSizeConflict = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
    	if (BENCHMARK_EMBEDDED) {
    		Log.i(TAG, "Franzi's benchmarking time: onCreate() start : " 
        ...
        mCalled = true;
        if (BENCHMARK_EMBEDDED) {
        	Log.i(TAG, "Franzi's benchmarking time: onCreate() end : " 
        			+ SystemClock.elapsedRealtimeNanos());
        }
    }
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        mCalled = true;
        if (BENCHMARK_EMBEDDED) {
        	Log.i(TAG, "Franzi's benchmarking time: onPostCreate() : " 
        			+ SystemClock.elapsedRealtimeNanos());
        }
    }
     ...
     * Added by Franzi.
     * 
     * Starts an Activity embedded inside of this one, with
     * the given display options.
     */
    public void startEmbeddedActivity(Intent intent, Bundle layoutParams, int id) {
    	// Embedded Activities don't return results.
    	mInstrumentation.execStartEmbeddedActivity(
                    this, mMainThread.getApplicationThread(), mToken, this,
                    intent, layoutParams, id);
    }
    
    /**
     * Added by Franzi.
     * 
     * Refreshes an Activity embedded inside of this one, with
     * the given display options.
     */
    public void refreshEmbeddedActivity(int id, Bundle layoutParams) {
    	mInstrumentation.execRefreshEmbeddedActivity(
                    this, mMainThread.getApplicationThread(), mToken, this,
                    id, layoutParams);
    }
     ...
     * Added by Franzi.
     * 
     * Removes an Activity embedded inside of this one.
     */
    public void removeEmbeddedActivity(int id) {
    	mInstrumentation.execRemoveEmbeddedActivity(
                    this, mMainThread.getApplicationThread(), mToken, this,
                    id);
    }
    ...
    /**
     * Added by Franzi: returns whether this Activity has a size conflict
     * (i.e., it requested a minimum size it didn't get when embedded)
     */
    public boolean fullyVisible() {
    	Rect frame = new Rect();
        getWindow().getDecorView().getWindowVisibleDisplayFrame(frame);
        boolean windowFullyVisible = (mExpectedHeight <= frame.height())
        							&& (mExpectedWidth <= frame.width());        
    	return (!mHasSizeConflict && windowFullyVisible);
    }
    
    /**
     * Added by Franzi: set expected height and width so
     * hasSizeConflict can determine if window isn't fully visible
     */
    public void setExpectedWindowDimensions(int height, int width) {
    	mExpectedHeight = height;
    	mExpectedWidth = width;
    }
    
    /**
     * Added by Franzi: set whether this Activity has a size conflict
     * (i.e., it requested a minimum size it didn't get when embedded)
     */
    public void setSizeConflict(boolean conflict) {
    	mHasSizeConflict = conflict;
    }
