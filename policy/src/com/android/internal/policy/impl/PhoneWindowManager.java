...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /** {@inheritDoc} */
    @Override
    public long interceptMotionBeforeDispatching(WindowState win, MotionEvent event, int policyFlags) {    	
    	mWindowManagerFuncs.checkRequestWindowFocus(win);  	
    	
    	// Finally, let this event fall through to the application
    	return 0;
    ...
    void setAttachedWindowFrames(WindowState win, int fl, int adjust,
        ...
        } else if (attrs.type == TYPE_EMBEDDED_ACTIVITY) {
        	// Added by Franzi: handle embedded activities differently.
        	// They should have at most the size specified by their
        	// parent; of course they can display smaller inside.
        	// (In ActivityThread, we transfered the parent specified size
        	// to the LayoutParams of this window, so just use that).
        	
        	// Since we might have multiple embedding, need to make sure
        	// to limit size of frames based on attached.
        	int left = attrs.x;
        	int top = attrs.y;
        	int right = attrs.x + attrs.width;
        	int bottom = attrs.y + attrs.height;
        	Rect parentFrame = attached.getDisplayFrameLw();
        	df.left = cf.left = vf.left = Math.max(left, parentFrame.left);
            df.top = cf.top = vf.top = Math.max(top, parentFrame.top);
            df.right = cf.right = vf.right = Math.min(right, parentFrame.right);
            df.bottom = cf.bottom = vf.bottom = Math.min(bottom, parentFrame.bottom);
        	
            /*df.left = cf.left = vf.left = attrs.x;
            df.top = cf.top = vf.top = attrs.y;
            df.right = cf.right = vf.right = attrs.x + attrs.width;
            df.bottom = cf.bottom = vf.bottom = attrs.y + attrs.height;*/
        } else {
    ...
    public void layoutWindowLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        if (!isDefaultDisplay) {
            if (attached != null) {
                ...
                setAttachedWindowFrames(win, fl, adjust, attached, true, pf, df, cf, vf, attrs);
            } else {
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_FULLSCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                setAttachedWindowFrames(win, fl, adjust, attached, false, pf, df, cf, vf, attrs);
            } else {
                if (DEBUG_LAYOUT)
