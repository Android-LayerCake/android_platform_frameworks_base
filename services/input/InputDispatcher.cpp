...
bool InputDispatcher::dispatchMotionLocked(
    ...
    // Added by Franzi: copied for intercept key case
    // Handle case where the policy asked us to try again later last time.
    if (entry->interceptMotionResult == MotionEntry::INTERCEPT_MOTION_RESULT_TRY_AGAIN_LATER) {
        if (currentTime < entry->interceptMotionWakeupTime) {
            if (entry->interceptMotionWakeupTime < *nextWakeupTime) {
                *nextWakeupTime = entry->interceptMotionWakeupTime;
            }
            return false; // wait until next wakeup
        }
        entry->interceptMotionResult = MotionEntry::INTERCEPT_MOTION_RESULT_UNKNOWN;
        entry->interceptMotionWakeupTime = 0;
    }
	
    // Give the policy a chance to intercept the event (if pointer only).
    if (isPointerEvent && (entry->interceptMotionResult == MotionEntry::INTERCEPT_MOTION_RESULT_UNKNOWN)) {
        if (entry->policyFlags & POLICY_FLAG_PASS_TO_USER) {
            CommandEntry* commandEntry = postCommandLocked(
	    ...
	    int32_t displayId = entry->displayId;
            ...
            commandEntry->touchedWindowHandle = findTouchedWindowAtLocked(displayId, x, y);
...
void InputDispatcher::doInterceptMotionBeforeDispatchingLockedInterruptible(
    ...
    event.initialize(entry->deviceId, entry->source, entry->action, entry->flags,
                    entry->edgeFlags, entry->metaState, entry->buttonState, 0, 0,
    ...
    nsecs_t delay = mPolicy->interceptMotionBeforeDispatching(commandEntry->touchedWindowHandle,
			&event, entry->policyFlags);
...
InputDispatcher::MotionEntry::MotionEntry(nsecs_t eventTime,
		...
		interceptMotionResult(MotionEntry::INTERCEPT_MOTION_RESULT_UNKNOWN),
		interceptMotionWakeupTime(0) {
