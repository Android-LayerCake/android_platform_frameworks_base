...
final class ActivityStack {
    ...
    static final boolean BENCHMARK_EMBEDDED = false;
    ...
    private final void startPausingLocked(boolean userLeaving, boolean uiSleeping) {
        ...
        if (prev.app != null && prev.app.thread != null) {
            ...
            try {
                ...
                // Added by Franzi: pause all embedded activities
                for (ActivityRecord childR : prev.embeddedActivities) {
                	childR.app.thread.schedulePauseActivity(childR.appToken, 
                			prev.finishing /* fate share with parent */, userLeaving,
                			childR.configChangeFlags /* not sure about this one... */);
                }
                if (mMainStack) {
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            try {
                ...
                // Added by Franzi: resume all embedded activities
                for (ActivityRecord childR : next.embeddedActivities) {
                	childR.app.thread.scheduleResumeActivity(childR.appToken, 
                			mService.isNextTransitionForward() /* not sure about this one... */);
                }
    ...
    ActivityInfo resolveActivity(Intent intent, String resolvedType, int startFlags,
            String profileFile, ParcelFileDescriptor profileFd, int userId, IBinder parent,
            boolean fullscreen) {
    	// Added by Franzi: see if starter is embedded 
    	ComponentName embeddedParent = null;
    	ActivityRecord parentRecord = ActivityRecord.forToken(parent);
    	if (parentRecord != null && parentRecord.parentActivity != null
    			&& fullscreen /* if an EMBEDDED activity is being started, this is
    			 			    okay... only worried about full-screen starts */) {
    		embeddedParent = parentRecord.realActivity;
    	}
        ...
        try {
            ResolveInfo rInfo =
                AppGlobals.getPackageManager().resolveIntentWithParent(
                        intent, resolvedType,
                        PackageManager.MATCH_DEFAULT_ONLY
                                    | ActivityManagerService.STOCK_PM_FLAGS, userId, embeddedParent);
        ...
        if (aInfo != null) {
        	// Franzi: if it's for the EmbeddedRedirectCheckActivity, we
            // need to save the intended component, if there is one
        	if (aInfo.applicationInfo.packageName.equals("android")
        		&& aInfo.name.equals("com.android.internal.app.EmbeddedRedirectCheckActivity")) {
	        	ComponentName specifiedComponent = intent.getComponent();
	        	if (specifiedComponent != null) {
	        		intent.putExtra("EmbeddedRedirectCheckActivity_PackageName", 
	        				specifiedComponent.getPackageName());
	        		intent.putExtra("EmbeddedRedirectCheckActivity_ClassName", 
	        				specifiedComponent.getClassName());
	        	}
	        	intent.putExtra("EmbeddedRedirectCheckActivity_ParentPackageName", 
        				embeddedParent.getPackageName());
	        	intent.putExtra("EmbeddedRedirectCheckActivity_ParentClassName", 
        				embeddedParent.getClassName());
        	}
    ...
    // Added by Franzi
    final int removeEmbeddedActivity(IApplicationThread caller, int callingUid,
    		int embeddedId, IBinder resultTo, 
            String resultWho, int startFlags, String profileFile, 
        ...
        synchronized (mService) {
            ...
            // Get record for container activity
            ActivityRecord parentRecord = ActivityRecord.forToken(resultTo);
            
            // Find topmost container (should always be highest parent, so
            // if this is a child of a child, need to step up)
            if (parentRecord.parentActivity != null) {
            	parentRecord = parentRecord.parentActivity;
            }
            
            // Find the specified child
            ActivityRecord childRecord = null;
            for (ActivityRecord child : parentRecord.embeddedActivities) {
            	if (child.embeddedId == embeddedId) {
            		childRecord = child;
            	}
            }
            if (childRecord == null) {
            	return ActivityManager.REFRESH_CHILD_NOT_FOUND;
            }
	    	
	    	// Actually remove Activity
	    	try {
		    	childRecord.app.thread.scheduleRemoveEmbeddedActivity(childRecord.appToken);
	    	} catch (RemoteException e) {
                Slog.w(TAG, "Exception when removing activity "
                        + childRecord.intent.getComponent().flattenToShortString(), e);
    ...
    // Added by Franzi
    final int refreshEmbeddedActivity(IApplicationThread caller, int callingUid,
    		int embeddedId, IBinder resultTo, 
            String resultWho, int startFlags, String profileFile, 
            ParcelFileDescriptor profileFd, WaitResult outResult, Configuration config,
            Bundle layoutParams, int userId) { 
        ...
        synchronized (mService) {
            ...
            // Keep track of layout params (so if this activity replaces
            // itself with another one using FLAG_ACTIVITY_EMBEDDED, new one
            // can inherit these params
            childRecord.layoutParams = layoutParams;
	    	
	    	// Actually refresh Activity
	    	try {
		    	childRecord.app.thread.scheduleRefreshEmbeddedActivity(childRecord.appToken, layoutParams);
	    	} catch (RemoteException e) {
                Slog.w(TAG, "Exception when refreshing activity "
                        + childRecord.intent.getComponent().flattenToShortString(), e);
                return ActivityManager.START_CANCELED;
            }
    ...
    // Added by Franzi
    final int startEmbeddedActivity(IApplicationThread caller, int callingUid,
            Intent intent, String resolvedType, IBinder resultTo, 
            String resultWho, int startFlags, String profileFile, 
            ParcelFileDescriptor profileFd, WaitResult outResult, Configuration config,
            Bundle layoutParams, int userId, int embeddedId,
            IBinder realParentToken /* only non-null if this is a replacement call */) {  
    	
    	// Note: resultTo should be a token for the containing Activity
        ...
        ActivityInfo aInfo = resolveActivity(intent, resolvedType, startFlags,
                profileFile, profileFd, userId, resultTo, false);
    	
    	/* This needs to do the following (like normal activity start thread, 
    	 * but different... don't want tasks and all that involved):
    	 * 
    	 * (1) make sure intent resolves, and check permission to start this activity
    	 * (2) create an activity record
    	 * (3) determine which app this activity is in (maybe need to start its process?)
    	 * (4) scheduleLaunchActivity with the specified record
    	 * 
    	 */
        synchronized (mService) {
        	int callingPid;
            ...
            if (err == ActivityManager.START_SUCCESS) {
                final int uid = aInfo != null ? UserHandle.getUserId(aInfo.applicationInfo.uid) : 0;
                Slog.i(TAG, "START u" + uid + " {" + intent.toShortString(true, true, true, false)
                        + "} from pid " + (callerApp != null ? callerApp.pid : callingPid));
            }
            ...
            ActivityRecord r = new ActivityRecord(mService, this, callerApp, callingUid,
                    intent, resolvedType, aInfo, mService.mConfiguration,
                    null, resultWho, -1, componentSpecified);
            
            //showAskCompatModeDialogLocked(r);
            r.compat = mService.compatibilityInfoForPackageLocked(r.info.applicationInfo);  
            
            // If realParentToken isn't null, then this Activity is being replaced.
            // It was previously resultTo, which is now calling startActivity() to replace
            // itself, so resultTo is not really a token for the parent as previously assumed.
            IBinder realParent = (realParentToken != null) ? realParentToken : resultTo;
            
            // Get record for container activity
            ActivityRecord parentRecord = ActivityRecord.forToken(realParent);
            r.immediateParentActivity = parentRecord;
            ...
            // Record that this (top-level) container activity has embedded children 
            // (for lifecycle fate sharing)
            parentRecord.embeddedActivities.add(r);
            
            // Record (top-level) container as parent of this child
            r.parentActivity = parentRecord;
            
            // Keep track of ID (so parents can reference this activity)
            r.embeddedId = embeddedId;
	    	...
	    	// (3): If process doesn't need to be started, will end up in finishStartingEmbeddedActivity()
	    	startProcessIfNecessary(r);	    	
    ...
    // Added by Franzi
    final void finishStartingEmbeddedActivity(ActivityRecord r, ProcessRecord app) {   	           
    	r.app = app;	    	
        
        // (4)
    	try {
	    	app.thread.scheduleLaunchEmbeddedActivity(new Intent(r.intent), r.appToken,
                    System.identityHashCode(r), r.info, r.compat, r.icicle, r.immediateParentActivity.appToken, 
                    r.parentActivity.appToken, r.embeddedId, r.layoutParams);
    	} catch (RemoteException e) {
            Slog.w(TAG, "Exception when starting activity "
                    + r.intent.getComponent().flattenToShortString(), e);
        }    	
    ...
    // Added by Franzi
    private final void startProcessIfNecessary(ActivityRecord r) {
        // Is this activity's application already running?
        ProcessRecord app = mService.getProcessRecordLocked(r.processName,
                r.info.applicationInfo.uid);
        ...
        if (app != null && app.thread != null) {
            app.addPackage(r.info.packageName);
            finishStartingEmbeddedActivity(r, app);
            return;
        }

        // Process wasn't started, need to do it now 
        // (will call back into finishStartingEmbeddedActivity() later)
        mService.startProcessLocked(r.processName, r.info.applicationInfo, true, 0,
                "activity", r.intent.getComponent(), false, false);
    }
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
    	...
    	if (BENCHMARK_EMBEDDED) {
    		Log.i(TAG, "Franzi's benchmarking time: startActivityMayWait() : " 
    			+ SystemClock.elapsedRealtimeNanos());
    	}
        
        // Added by Franzi
        // If the Activity is being started by an embedded Activity
        // and has FLAG_ACTIVITY_EMBEDDED set, we're actually launching an embedded Activity
    	// (replacing old child with new child)
        if ((intent.getFlags() & Intent.FLAG_ACTIVITY_EMBEDDED) != 0) {
        	ActivityRecord oldChildRecord = ActivityRecord.forToken(resultTo);
        	// only do something if parent is embedded, otherwise can ignore the flag
        	if (oldChildRecord != null && oldChildRecord.parentActivity != null) {
        		// Save metadata
        		IBinder realParentToken = oldChildRecord.parentActivity.appToken;
        		Bundle layoutParams = oldChildRecord.layoutParams;
        		int embeddedId = oldChildRecord.embeddedId;
        		
        		// Remove the old child
        		// (note that metadata in ActivityRecords will update in childActivityDestroyed() callback
    	    	try {
    		    	oldChildRecord.app.thread.scheduleRemoveEmbeddedActivity(oldChildRecord.appToken);
    	    	} catch (RemoteException e) {
                    Slog.w(TAG, "Exception when removing activity "
                            + oldChildRecord.intent.getComponent().flattenToShortString(), e);
                    // Oh well?
                }
        		
        		// We're actually launching an embedded Activity (metadata will be updated there
    	    	// for new child and for parent)
    	    	return startEmbeddedActivity(caller, callingUid, intent, resolvedType, resultTo, 
    	    			resultWho, startFlags, profileFile, profileFd, outResult, config, 
    	    			layoutParams, userId, embeddedId, realParentToken);
        	}
        }
        ...
        // Collect information about the target of the Intent.
        ActivityInfo aInfo = resolveActivity(intent, resolvedType, startFlags,
                profileFile, profileFd, userId, resultTo, true);
            ...
            synchronized (mService) {
                ...
                for (int i=0; i<intents.length; i++) {
                    ...
                    ActivityInfo aInfo = resolveActivity(intent, resolvedTypes[i],
                            0, null, null, userId, resultTo, true);
    ...
    private final void stopActivityLocked(ActivityRecord r) {
        ...
        if (r.app != null && r.app.thread != null) {
            ...
            try {
                ...
                // Added by Franzi: stop all embedded activities
                for (ActivityRecord childR : r.embeddedActivities) {
                	childR.app.thread.scheduleStopActivity(childR.appToken,
                			r.visible /* ? */, childR.configChangeFlags /* ? */);
                }
                if (mService.isSleeping()) {
                    r.setSleeping(true);
                }
        ...
        if (hadApp) {
            ...
            try {
                ...
                r.app.thread.scheduleDestroyActivity(r.appToken, r.finishing,
                        r.configChangeFlags);
                // Actually: embedded activities will get destroyed when their
                // associated EmbeddedActivityView is removed from the window
                // Added by Franzi: destroy all embedded activities
                //for (ActivityRecord childR : r.embeddedActivities) {
                //	childR.app.thread.scheduleDestroyActivity(childR.appToken, 
                //			r.finishing /* fate share with parent */, 
                //			childR.configChangeFlags /* not sure about this one... */);
                //}
            } catch (Exception e) {
    ...
    /**
     * Added by Franzi.
     * Need to remove child from parent's list.
     */
    final void childActivityDestroyed(IBinder containerToken, IBinder childToken) {
    	synchronized (mService) {
            final long origId = Binder.clearCallingIdentity();
           
            ActivityRecord r = ActivityRecord.forToken(containerToken);
            
            // Find topmost container (should always be highest parent, so
            // if this is a child of a child, need to step up)
            if (r.parentActivity != null) {
            	r = r.parentActivity;
            }
            
            // Find child
            for (ActivityRecord child : r.embeddedActivities) {
            	if (childToken == child.appToken) {
            		r.embeddedActivities.remove(child);
            		child.parentActivity = null;
            		break;
            	}
            }
